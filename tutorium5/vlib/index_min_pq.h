...
class IndexMinPQ {
   ...
   public:
    // constructor
    IndexMinPQ(const int _capacity = 1) : capacity(_capacity), n(0) {
    ...
    // copy constructor
    IndexMinPQ(const IndexMinPQ& mq) : capacity(mq.capacity), n(mq.n) {
        keys = new T[capacity + 1];
        pq = new int[capacity + 1];
        qp = new int[capacity + 1];
    ...
    // move constructor
    IndexMinPQ(IndexMinPQ&& mq)
        : capacity(mq.capacity), n(mq.n), keys(mq.keys), pq(mq.pq), qp(mq.qp) {
        mq.capacity = 0;
        mq.n = 0;
        mq.keys = nullptr;
        mq.pq = nullptr;
    ...
    bool contains(const int i) const { return (qp[i] != -1); }
    ...
    void decrease_key(const int i, const T& key) {
        ...
        if (keys[i] == key)
            throw logic_error(
                "calling decrease_key() with a key equal to the key in the "
                "priority queue");
        if (keys[i] < key)
            throw logic_error(
                "calling decrease_key() with a key strictly greater than the "
    ...
    void increase_key(const int i, const T& key) {
        ...
        if (keys[i] == key)
            throw logic_error(
                "calling increase_key() with a key equal to the key in the "
                "priority queue");
        if (keys[i] > key)
            throw logic_error(
                "calling increase_key() with a key strictly less than the key "
