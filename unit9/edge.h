...
class Edge {
   ...
   public:
    // constructor
    Edge(int _v=0, int _w=0, double _edge_weight=std::numeric_limits<double>::infinity()) : v(_v), w(_w), edge_weight(_edge_weight) {}
    ...
    int other(const int vertex) const {
        if (vertex == v)
            ...
            throw std::logic_error("Illegal endpoint");
    }
