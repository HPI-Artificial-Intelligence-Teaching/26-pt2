...
class EdgeWeightedDigraph {
   ...
   public:
    ...
    EdgeWeightedDigraph(const int V) : no_vertices(V),
                                       no_edges(0) {
        if (V < 0) throw std::logic_error("Number of vertices must be non-negative");
    ...
    EdgeWeightedDigraph(std::ifstream&);
...
extern std::ostream& operator<<(std::ostream&, const EdgeWeightedDigraph&);
