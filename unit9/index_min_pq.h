...
class IndexMinPQ {
   ...
   public:
    ...
    const T& key_of(const int i) const {
        if (!contains(i)) 
            throw std::logic_error("index is not in the priority queue");
        else 
            return keys[i];
    }
    ...
    int min_index() const {
        if (is_empty()) throw std::logic_error("Priority queue underflow");
        return (pq[1]);
    }
    ...
    void insert(const int i, const T& key) {
        if (contains(i)) throw std::logic_error("index is already in the priority queue");
    ...
    void change_key(const int i, const T& key) {
        if (!contains(i)) throw std::logic_error("index is not in the priority queue");
    ...
    void decrease_key(const int i, const T& key) {
        if (!contains(i)) throw std::logic_error("index is not in the priority queue");
        if (keys[i] == key) throw std::logic_error("calling decrease_key() with a key equal to the key in the priority queue");
        if (keys[i] < key) throw std::logic_error("calling decrease_key() with a key strictly greater than the key in the priority queue");
    ...
    void increase_key(const int i, const T& key) {
        if (!contains(i)) throw std::logic_error("index is not in the priority queue");
        if (keys[i] == key) throw std::logic_error("calling increase_key() with a key equal to the key in the priority queue");
        if (keys[i] > key) throw std::logic_error("calling increase_key() with a key strictly less than the key in the priority queue");
