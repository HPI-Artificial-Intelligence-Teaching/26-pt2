...
class BellmanFordSP {
    ...
    void relax(const EdgeWeightedDigraph& dg, const int v, Queue<int>& queue, bool* on_queue, int& cost) {
        for (auto& e : dg.adj(v)) {
            int w = e.to();
            if (dist_to[w] > dist_to[v] + e.weight() + std::numeric_limits<double>::epsilon()) {
   ...
   public:
    ...
    bool has_path_to(const int v) const {
        validate_vertex(v);
        return (dist_to[v] < std::numeric_limits<double>::infinity());
    }

    // returns a shortest path from the source vertex to vertex v
    const Stack<Edge> path_to(const int v) const {
        validate_vertex(v);
        if (has_negative_cycle()) throw std::logic_error("Negative cost cycle exists");
